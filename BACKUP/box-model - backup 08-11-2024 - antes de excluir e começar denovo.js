...
function atualizarTotal() {
    ...
    let limpezaSimplesSelecionada = false;
    let outrosServicosSelecionados = false;
    ...
    selecoes.each(function(){
        const nomeServico = $(this).find('.p-single').text().trim();
        ...
        // Verifica se o serviço é "Limpeza Simples
        /*
        if (nomeServico === "Limpeza Simples de Sobrancelhas") {
            limpezaSimplesSelecionada = true;
        } else {
            // Se não for "Limpeza Simples", verifica se outros serviços estão selecionados
            if (!outrosServicosSelecionados) {
                outrosServicosSelecionados = true;
            }
        }*/
    ...
    // Se a Limpeza Simples foi selecionada e outros serviços também estão selecionados, zerar a duração da Limpeza Simples
    
    if (limpezaSimplesSelecionada && outrosServicosSelecionados) {
        console.log("Limpeza Simples foi selecionada junto com outros serviços. ZERANDO a duração.");

        // Atualizar a duração da Limpeza Simples para 0:00 no DOM
        const selecaoLimpezaSimples = $('.wraper-resumo .selecao-single')
            .filter(function() {
                return $(this).find('.p-single').text().trim() === 'Limpeza Simples de Sobrancelhas';
            });

        selecaoLimpezaSimples.each(function() {
           $(this).find('.duracao span').html('<i class="fa-solid fa-clock" aria-hidden="true"></i> 0:00'); // Atualiza a duração para 0:00
        });

        // Zera a duração no localStorage 
        const clienteId = getClienteId();
        localStorage.setItem(`cliente_${clienteId}_servico_Limpeza Simples de Sobrancelhas`, JSON.stringify({
            nomeServico: 'Limpeza Simples',
            precoServico: precoTexto,
            duracaoServico: '0:00',
        }));
    }
