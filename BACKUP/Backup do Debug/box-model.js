...
function atualizarResumoSelecao(checked, checkboxId) {
    ...
    for (const [id, servico] of Object.entries(servicosSelecionadosAgenda)) {
        ...
        console.log(`Quantidade de contêineres encontrados: ${$container.length} para cliente ${clienteId}`);
        ...
        // Valida se o ID do contêiner corresponde ao ID do cliente
        const containerId = $container.attr("class").match(/agenda-resumo-cliente-(\d+)/)?.[1];

        console.log(`Debug - ID do cliente: ${clienteId}, ID do contêiner extraído: ${containerId}`);

        if (containerId !== clienteId.toString()) {
            console.error(`ID do contêiner (${containerId}) não corresponde ao ID do cliente (${clienteId}). Serviço não será adicionado.`);
            continue;
        } else {
            console.log(`IDs correspondem: Contêiner (${containerId}), Cliente (${clienteId}).`);
        }

        // Verifica se o serviço já foi adicionado
        const existingService = $container.find(`[data-id="agenda-servico-${servicoNome}"]`);

        console.log(`Procurando serviço com data-id: agenda-servico-${id}. Encontrado: ${existingService.length}`);
        ...
        if (novoServicoHTML) {
            ...
            console.log(`Serviço ${servicoNome} adicionado ao contêiner do cliente ${clienteId}.`);
        }
    }
