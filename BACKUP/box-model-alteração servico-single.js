...
function checkBox() {
    ...
    /*Captura as caixas de seleção de quantidade*/
    $('.single-quantidade').click(function() {
        const selectedValue = parseInt($(this).find('input').val().replace('qnt_cliente_', '')); // Pega o valor selecionado
        // Remove a classe 'qnt-clientes-select' de todas as divs antes de aplicar a nova seleção
    ...
    $('.servico-single').click(function() {
        ...
        if (checked) {
            ...
            localStorage.setItem(`servico_${nomeServico}`, JSON.stringify(servicoData));
            ...
            $('.icone-lixeira').click(function() {
                ...
                localStorage.removeItem(`servico_${nomeServico}`); // Remove o serviço do localStorage
